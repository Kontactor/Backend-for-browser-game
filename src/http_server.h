...
protected:
    ...
    void DoWrite(SessionBase* session, http::response<Body, Fields>& response);

    void Write(http_handler::ServerResponse&& response);
...
template <class RequestHandler>
void Session<RequestHandler>::HandleRequest(
    HttpRequest&& request,
